...
export default function Habits({ habits, setHabits, t }) {
  ...
  return (
    <div>
      <PageHdr label="Habits" description="Keep small routines visible and mark what you complete today." action={{ label: "Add habit", onClick: () => setModal(true) }} t={t} />
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 14 }}>
        {habits.map((habit) => (
          <div key={habit.id} style={{ ...s.card, borderLeft: habit.done ? `4px solid ${t.accent}` : `4px solid ${t.borderLight}` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: t.textMutedMore, marginTop: 2 }}>{habit.category}</div>
                </div>
              </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, color: t.accent, marginBottom: 12, fontWeight: 700 }}><Icon.fire /> {habit.streak} day streak</div>
